...
export function getInstallStatus() {
  return {
    ...
    ubuntu: (() => {
      try {
        const result = execSync('proot-distro list', { stdio: 'pipe' }).toString();
        return result.includes('ubuntu') && fs.existsSync(PROOT_HOME);
      } catch {
        return false;
      }
    })(),
    openClawInProot: (() => {
      try {
        execSync('proot-distro login ubuntu -- which openclaw', { stdio: 'pipe' });
        return true;
      } catch {
        return false;
      }
...
`;
  ...
  const prootBypassPath = path.join(PROOT_HOME, '.openclawd', 'bionic-bypass.js');
  ...
  try {
    ...
    const prootBashrc = path.join(PROOT_HOME, '.bashrc');
    const exportLine = 'export NODE_OPTIONS="--require /root/.openclawd/bionic-bypass.js"';
...
export function runInProot(command) {
  const nodeOptions = '--require /root/.openclawd/bionic-bypass.js';
